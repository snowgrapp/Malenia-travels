export class Inventory {
    constructor(scene) {
        ...
        this.items = {};
        this.inventoryBackground = scene.add
            .rectangle(
                scene.scale.width / 2,
        ...
        const startX =
            ...
            (cols * (slotSize + padding)) / 2 +
            slotSize / 2;
        const startY =
            this.inventoryBackground.y -
            (rows * (slotSize + padding)) / 2 +
            slotSize / 2;
        ...
        for (let row = 0; row < rows; row++) {
            for (let col = 0; col < cols; col++) {
                ...
                const slot = scene.add
                    .rectangle(x, y, slotSize, slotSize, 0x333333)
    ...
    setupDragEvents() {
        ...
        this.scene.input.on("drop", (pointer, gameObject) => {
            ...
            const type = Object.keys(this.items).find(
                (key) => this.items[key].icon === gameObject
            );
            if (!type) return;

            if (
                this.scene.toolBox &&
                this.scene.toolBox.container
                    .getBounds()
    ...
    transferToToolBox(type) {
        ...
        const newIcon = this.scene.add
            .sprite(0, 0, item.texture, item.frame)
            .setScrollFactor(0)
            .setScale(1.8)
        ...
        newIcon.on("pointerdown", (pointer) => {
            if (
                ...
                this.addItem(type, item.texture, item.frame);
                newIcon.destroy();
                toolBox.toolIcons = toolBox.toolIcons.filter(
                    (i) => i !== newIcon
                );
